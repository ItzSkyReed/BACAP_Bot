...
from Database import DB_Advancement, DB_AdvancementAltDescriptions, DB_Trophy, DB_Reward, DB_WB_Addon
...
def __load_parser():
    ...
    cereal_dedication = Datapack(name=CEREAL_DEDICATION_DATAPACK_NAME, path=ASSETS_FOLDER / r"datapacks/cereal_dedication", adv_type_manager=cereal_dedication_manager,
                                 reward_namespace="bacap_rewards",
                                 technical_tabs="technical")

    cereal_dedication_hardcore = Datapack(name=CEREAL_DEDICATION_HARDCORE_DATAPACK_NAME, path=ASSETS_FOLDER / r"datapacks/cereal_dedication_hardcore", adv_type_manager=cereal_dedication_manager,
                                          technical_tabs="technical")
...
def _is_advancement_in_database(advancement_mc_path: str) -> bool:
    return bool(__run_async_sync(DB_Advancement.get_adv_by_mc_path(advancement_mc_path)))


def _get_advancement_id_by_mc_path(advancement_mc_path: str) -> int | None:
    adv = __run_async_sync(DB_Advancement.get_adv_by_mc_path(advancement_mc_path))
    if adv:
        return adv.id
    else:
        return None
...
def __save_advancement_sync(advancement: DB_Advancement) -> DB_Advancement:
    return __run_async_sync(advancement.save())


def __save_trophy_sync(trophy: DB_Trophy) -> DB_Trophy:
    return __run_async_sync(trophy.save())


def __save_reward_sync(reward: DB_Reward) -> DB_Reward:
    return __run_async_sync(reward.save())


def __save_wb_sync(wb: DB_WB_Addon) -> DB_WB_Addon:
...
def _save_alt_description_field(description: str, field_name: str) -> int:
    """
    ...
    """
    alt_desc = DB_AdvancementAltDescriptions()
...
def _process_existing_advancement(
    ...
    if adv.description != normal_adv.description and normal_adv.alt_descriptions_id is None:
        # Save new alternativeё description for this advancement
